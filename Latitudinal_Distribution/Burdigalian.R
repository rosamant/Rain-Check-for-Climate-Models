...
Bur_sf <- st_as_sf(Bur, coords = c("lon", "lat"), crs = 4326)
...
Bur_mask <- Bur_sf[st_within(Bur_sf, mask_sf, sparse = FALSE), ]
...
# NGR Hist
ngr_Bur_hist <- final_df2 %>%
  filter(Age >= 18.45, Age <= 20.45) %>%
...
Bur_pe_min <- min(Bur_mask$pe, na.rm = TRUE)
Bur_pe_max <- max(Bur_mask$pe, na.rm = TRUE)

pe_Bur_hist <- Bur_mask %>%
  ...
  mutate(lat = as.numeric(as.character(lat_bin)),
         model_scaled = (model_raw - Bur_pe_min) / (Bur_pe_max - Bur_pe_min))

common_Bur_lats <- seq(min(ngr_Bur_hist$lat), max(ngr_Bur_hist$lat), by = 1)
Bur_interp_obs <- approx(ngr_Bur_hist$lat, ngr_Bur_hist$obs_scaled, xout = common_Bur_lats, rule = 2)$y
Bur_interp_model <- approx(pe_Bur_hist$lat, pe_Bur_hist$model_scaled, xout = common_Bur_lats, rule = 2)$y
...
png(filename = "Burdigalian_PE_DJF.png", width = 6000, height = 4500, res = 600)

par(mar = c(4.5, 4.5, 1.5, 1.5), cex.lab = 1.5, cex.axis = 1.5) 

plot(pe_Bur_hist$lat, pe_Bur_hist$model_scaled, type = "b", pch = 16, col = "gray60", lty = 1, lwd = 2,
...
polygon(x = c(common_Bur_lats, rev(common_Bur_lats)), y = c(Bur_interp_model, rev(Bur_interp_obs)), density = 20, angle = 45,
        col = "#F2B32C", border = NA)
lines(pe_Bur_hist$lat, pe_Bur_hist$model_scaled, col = "gray60", lty = 1, lwd = 2)
points(ngr_Bur_hist$lat, ngr_Bur_hist$obs_scaled, pch = 18, col = "black", cex = 1.5)
lines(ngr_Bur_hist$lat, ngr_Bur_hist$obs_scaled, col = "black", lwd = 2, lty = 2)
arrows(x0 = ngr_Bur_hist$lat, y0 = ngr_Bur_hist$Q1_scaled, x1 = ngr_Bur_hist$lat,
       y1 = ngr_Bur_hist$Q3_scaled, angle = 90, code = 3, length = 0.04, col = "black", lwd = 1.2)
