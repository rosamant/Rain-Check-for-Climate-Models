...
Zan_sf <- st_as_sf(Zan, coords = c("lon", "lat"), crs = 4326)
...
Zan_mask <- Zan_sf[st_within(Zan_sf, mask_sf, sparse = FALSE), ]
...
# NGR Hist
ngr_Zan_hist <- final_df2 %>%
  filter(Age >= 3.6, Age <= 5.33) %>%
...
Zan_pe_min <- min(Zan_mask$pe, na.rm = TRUE)
Zan_pe_max <- max(Zan_mask$pe, na.rm = TRUE)

pe_Zan_hist <- Zan_mask %>%
  ...
  mutate(lat = as.numeric(as.character(lat_bin)),
         model_scaled = (model_raw - Zan_pe_min) / (Zan_pe_max - Zan_pe_min))

common_Zan_lats <- seq(min(ngr_Zan_hist$lat), max(ngr_Zan_hist$lat), by = 1)
...
png(filename = "Zanclean_PE_DJF.png", width = 6000, height = 4500, res = 600)

par(mar = c(4.5, 4.5, 1.5, 1.5), cex.lab = 1.5, cex.axis = 1.5) 

plot(pe_Zan_hist$lat, pe_Zan_hist$model_scaled, type = "b", pch = 16, col = "gray60", lty = 1, lwd = 2,
...
lines(pe_Zan_hist$lat, pe_Zan_hist$model_scaled, col = "gray60", lty = 1, lwd = 2)
points(ngr_Zan_hist$lat, ngr_Zan_hist$obs_scaled, pch = 18, col = "black", cex = 1.5)
lines(ngr_Zan_hist$lat, ngr_Zan_hist$obs_scaled, col = "black", lwd = 2, lty = 2)
arrows(x0 = ngr_Zan_hist$lat, y0 = ngr_Zan_hist$Q1_scaled, x1 = ngr_Zan_hist$lat,
       y1 = ngr_Zan_hist$Q3_scaled, angle = 90, code = 3, length = 0.04, col = "black", lwd = 1.2)
