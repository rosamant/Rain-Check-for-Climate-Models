...
Tor_TR <- nc_open("RScripts&Data/NetCDF/tfgSa.djf_runoff_data.nc")
Tor_LSM <- nc_open("RScripts&Data/NetCDF/tfgSa.qrparm.mask.nc")
names(Tor_TR$var)
names(Tor_LSM$var)

lon_vec <- ncvar_get(Tor_TR, "longitude")
lat_vec <- ncvar_get(Tor_TR, "latitude")
Tor_SR <- ncvar_get(Tor_TR, "srfRunoff_mm_srf")
Tor_SSR <- ncvar_get(Tor_TR, "subsrfRunoff_mm_srf")
Tor_R <- Tor_SSR + Tor_SR
Tor_lsm <- ncvar_get(Tor_LSM, "lsm")

nc_close(Tor_TR)
nc_close(Tor_LSM)

Tor_R_mm <- Tor_R * 86400

Tor <- expand.grid(lon = lon_vec, lat = lat_vec)
Tor$elevation <- as.vector(Tor_lsm)
Tor$R <- as.vector(Tor_R_mm)
...
poly_coords <- data.frame(lon = c(107, 137, 144, 114, 107),
                          lat = c(-30, -9, -9, -30, -30))
...
png(filename = "Tortonian_R_DJF.png", width = 5000, height = 5000, res = 600)
